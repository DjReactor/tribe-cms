...
- App Router under `src/app/`:
  ...
  - SEO wiring: middleware forwards `x-pathname` on every request; `not-found.tsx` uses it to apply DB-driven redirects (`redirects` collection — `from_path`/`to_path`/`type`, hit-counted) and to upsert misses into `seo_404_log` (`hit_count`/`last_seen`/`resolved`). Redirects only fire for paths that would otherwise 404 (they cannot shadow live routes). Schema for all of this reconciled in migration `1960000000`.
