  ...
  return (
    <div className="space-y-6">
        ...
        {ORDER.map((s) => (
          <Card key={s}>
            <CardContent className="pt-5">
              ...
              <p className="mt-1 text-2xl font-bold text-foreground">{counts[s] ?? 0}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-foreground">Failed &amp; dead-lettered</h2>
      ...
      <Card>
        <CardContent className="pt-6">
          {recent.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No failed or dead events.</p>
          ) : (
                ...
                <tbody>
                  {recent.map((r) => (
                    <tr key={r.id} className="border-b border-border last:border-0">
                      <td className="py-2 pr-4 font-medium text-foreground">{r.event}</td>
                      <td className="py-2 pr-4"><Badge variant={STATUS_VARIANT[r.status] || 'default'}>{r.status}</Badge></td>
                      <td className="py-2 pr-4 text-muted-foreground">{r.attempts}</td>
                      <td className="py-2 pr-4 max-w-xs truncate text-muted-foreground" title={r.last_error}>{r.last_error || '—'}</td>
                      <td className="py-2 text-muted-foreground">{r.updated ? new Date(r.updated).toLocaleString() : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
