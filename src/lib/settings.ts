...
export const getSettings = cache(async (): Promise<TemplateSettings & { id: string; template_config: any }> => {
  try {
    ...
    if (record) {
      return {
        ...
        locations_enabled: record.locations_enabled ?? false,
        ...
        services_display_mode: (record.services_display_mode as 'auto' | 'flat' | 'tree') || 'auto',
  ...
  return {
    ...
    services_display_mode: 'auto' as const,
    active_palette_id: '',
