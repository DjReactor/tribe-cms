...
- Global data props: `serviceAreas` and `locations` (active records) are passed to every page component, so a template can render a "Service Areas" / "Locations" section on any page. Templates are pure — they never query; the page wrapper fetches (via `getLocations()` / inline) and passes the prop. Gate optional sections on `locations.length > 0`. `locations` is empty everywhere whenever the `locations_enabled` master switch is off, so count-based visibility doubles as the feature gate.
