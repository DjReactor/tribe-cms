...
- Discovery is fully dynamic: `../lib/template-registry.ts` reads `src/templates/*` via `fs.readdirSync`; `../lib/template-loader.ts` does `import(@/templates/<id>)` with fallback to `modern`. Never hardcode a template list.
