        ...
        <div className="max-w-5xl mx-auto px-6 py-16 md:py-24">
          <div className="flex flex-col md:flex-row gap-12 items-center">
            <div className="flex-1">
              <Link href="/services" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--tribe-brand)] hover:text-gray-900 mb-6 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Back to All Services
              </Link>
              <h1 className="font-heading text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                {service.icon && <span className="mr-4 text-[var(--tribe-brand)] inline-block">{service.icon}</span>}
                {service.name}
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/contact" className="px-8 py-4 bg-[var(--tribe-brand)] text-[var(--tribe-brand-text)] font-bold rounded-lg hover:opacity-90 transition-opacity">
                  Get a Quote
                </Link>
                {businessInfo.phone && (
                  <a href={`tel:${businessInfo.phone}`} className="px-8 py-4 bg-[var(--tribe-surface)] border border-gray-200 text-gray-900 font-bold rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <div className="bg-[var(--tribe-surface)] p-8 rounded-2xl border border-gray-200">
                ...
                <ul className="space-y-3">
                  {serviceAreas.map((area) => (
                    <li key={area.id}>
                      <Link href={`/${area.slug}`} className="flex items-center gap-2 text-gray-600 hover:text-[var(--tribe-brand)] transition-colors">
