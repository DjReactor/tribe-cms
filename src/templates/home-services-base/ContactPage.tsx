            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {businessInfo.phone && (
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-[var(--color-accent)] mb-4 shadow-sm">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                  </div>
                  <h3 className="font-semibold text-gray-900 mb-1">Phone</h3>
                  <a href={`tel:${businessInfo.phone}`} className="text-lg text-[var(--color-accent)] hover:underline font-medium">
                    {businessInfo.phone}
                  </a>
                </div>
              )}
              {businessInfo.email && (
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  <a href={`mailto:${businessInfo.email}`} className="text-lg text-[var(--color-accent)] hover:underline font-medium break-words">
                    {businessInfo.email}
                  </a>
                </div>
              )}
            ...
            {businessInfo.address && (
              <div className="bg-gray-50 p-8 rounded-2xl border border-gray-100">
                ...
                <p className="text-gray-700 text-lg mb-6 flex items-start gap-3">
                  <svg className="w-6 h-6 text-[var(--color-accent)] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                  {businessInfo.address}
                </p>
                {embedUrl ? (
                  ...
                  <a href={businessInfo.google_maps_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-white border border-gray-200 rounded-lg text-[var(--color-accent)] font-semibold hover:bg-gray-50 transition-colors">
                    View on Google Maps
