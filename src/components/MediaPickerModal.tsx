  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Select from Media Gallery" className="max-w-4xl max-h-[80vh] flex flex-col">
      <div className="flex justify-between items-center py-4 border-b">
         <label className="bg-primary hover:bg-primary text-primary-foreground px-4 py-2 rounded cursor-pointer transition-colors">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-4 gap-4">
            {mediaList.map((item) => (
              <div 
                key={item.id} 
                className="relative aspect-square border rounded cursor-pointer hover:border-ring overflow-hidden group"
